 ...
 * Exhaustion: 16.0 (flat)
 * Damage: If successful, inflicts damage * (3.0 + level) + 100 * level
 * Duration: Window of attack opportunity is (35 + level) ticks
 * Notes:
...
public class MortalDraw extends SkillActive
{
	...
	public void addInformation(List<String> desc, EntityPlayer player) {
		desc.add(getDamageDisplay(100, true) + "% + " + (100 * level));
	 ...
	 * Modified: Window is 35 + level ticks
	 */
	 ...
	 * Modified: New damage formula and damage source with IComboDamage tag
	 */
	@Override
	public float onImpact(EntityPlayer player, EntityLivingBase entity, float amount) {
		if (attackTimer > DELAY) {
			...
			// 新伤害公式：原始伤害 * (3.0 + level) + 100 * level
			float newDamage = amount * (3.0F + (float) level) + 100.0F * level;
			// 使用间接剑技伤害源，增加伤害标签
			entity.attackEntityFrom(DamageUtils.causeIndirectComboDamage(player, player), newDamage);
			return 0; // 取消原伤害事件，避免二次伤害
		} else {
			target = null;
		}
		return amount;
